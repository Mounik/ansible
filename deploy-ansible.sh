...
createAnsible(){
	echo ""
  mkdir -p ${ANSIBLE_DIR}
  echo "all:" > ${ANSIBLE_DIR}/hosts.yml
  echo "  vars:" >> ${ANSIBLE_DIR}/hosts.yml
  echo "    ansible_python_interpreter: /usr/bin/python3" >> ${ANSIBLE_DIR}/hosts.yml
  echo "  hosts:" >> ${ANSIBLE_DIR}/hosts.yml
  sudo podman ps -aq | awk '{system("sudo podman inspect -f \"{{.NetworkSettings.IPAddress}}:\" "$1)}' >> ${ANSIBLE_DIR}/hosts.yml
